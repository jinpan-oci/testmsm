...
class SaleOrder(models.Model):
    ...
    def preview_report_sale_order(self):
        if self:
        ...
        report = self.env['ir.actions.report']._get_report_from_name(reportname) or self.env['ir.actions.report']._get_report_from_name('sale.report_saleorder')

        if not report:
            raise UserError(_("Has not defined any report with the template name of 'ReporSaleOrderPreview' or 'sale.report_saleorder'"))
