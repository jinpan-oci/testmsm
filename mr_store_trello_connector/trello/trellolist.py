...
class List(TrelloBase):
    ...
    def __repr__(self):
        return force_str("<List %s>" % self.name)
    ...
    def list_cards(self, card_filter="open", actions=None, query=None):
        ...
        json_obj = self.client.fetch_json(
            "/lists/" + self.id + "/cards", query_params=query_params
        )
        return [Card.from_json(self, c) for c in json_obj]

    def list_cards_iter(
        self, card_filter="open", actions=None, query=None, limit=None, batch=300
    ):
        ...
        while True:
            ...
            cards = self.list_cards(
                card_filter=card_filter, actions=actions, query=query
            )
            n = len(cards)
            if n == 0:
                break
    ...
    def add_card(
        ...
        post_args = {
            ...
            "locationName": locationName if locationName else "",
        ...
        json_obj = self.client.fetch_json(
            "/cards", http_method="POST", post_args=post_args
        )
        return Card.from_json(self, json_obj)

    def archive_all_cards(self):
        self.client.fetch_json(
            "/lists/" + self.id + "/archiveAllCards", http_method="POST"
        )
    ...
    def fetch_actions(self, action_filter):
        ...
        """
        json_obj = self.client.fetch_json(
            "/lists/" + self.id + "/actions", query_params={"filter": action_filter}
        )
