...
class Member(TrelloBase):
    ...
    def __repr__(self):
        return force_str("<Member %s>" % self.id)

    def fetch(self):
        """Fetch all attributes for this member"""
        json_obj = self.client.fetch_json(
            "/members/" + self.id, query_params={"badges": False}
        )
    ...
    def get_boards(self, list_filter):
        ...
        organizations = {
            obj["idOrganization"]: self.client.get_organization(obj["idOrganization"])
            for obj in json_obj
            if obj["idOrganization"]
        }
        return [
            Board.from_json(
                trello_client=self.client,
                organization=organizations.get(obj["idOrganization"]),
                json_obj=obj,
            )
            for obj in json_obj
        ]
