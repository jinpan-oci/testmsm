...
from .customfield import CustomFieldDefinition
...
class Board(TrelloBase):
    ...
    def from_json(cls, trello_client=None, organization=None, json_obj=None):
        ...
        if organization is None:
            board = Board(
                client=trello_client, board_id=json_obj["id"], name=json_obj["name"]
            )
        else:
            board = Board(
                organization=organization,
                board_id=json_obj["id"],
                name=json_obj["name"],
            )
    ...
    def __repr__(self):
        return force_str("<Board %s>" % self.name)
    ...
    def get_custom_field_definitions(self):
        ...
        if self.customFieldDefinitions is None:
            ...
            self.customFieldDefinitions = CustomFieldDefinition.from_json_list(
                self, json_obj
            )
        return self.customFieldDefinitions

    def add_custom_field_definition(
        self, name, type, options=None, display_on_card=None, pos=None
    ):
    ...
    def update_custom_field_definition(
        self, custom_field_definition_id, name=None, display_on_card=None, pos=None
    ):
        ...
        if name:
            arguments["name"] = name
        if display_on_card is not None:
            arguments["display/cardFront"] = "true" if display_on_card else "false"
        if pos:
            arguments["pos"] = pos

        json_obj = self.client.fetch_json(
            "/customFields/{}".format(custom_field_definition_id),
            http_method="PUT",
            post_args=arguments,
        )
        return json_obj
    ...
    def get_custom_field_list_options(
        self, custom_field_definition_id, values_only=False
    ):
        ...
        json_obj = self.client.fetch_json(
            "customFields/{}/options".format(custom_field_definition_id),
            http_method="GET",
        )
        if values_only:
    ...
    def get_custom_field_list_option(self, custom_field_definition_id, option_id):
        ...
        json_obj = self.client.fetch_json(
            "customFields/{}/options/{}".format(custom_field_definition_id, option_id),
            http_method="GET",
        )
        # if values_only:
        # 	return [v for jo in json_obj for v in jo['value'].values()]
    ...
    def get_label(self, label_id):
        ...
        """
        json_obj = self.client.fetch_json("/boards/" + self.id + "/labels/" + label_id)
    ...
    def get_checklists(self, cards="all"):
        ...
        checklists = []
        json_obj = self.client.fetch_json(
            "/boards/" + self.id + "/checklists", query_params={"cards": cards}
        )
    ...
    def get_cards(self, filters=None, card_filter=""):
        ...
        """
        json_obj = self.client.fetch_json(
            "/boards/" + self.id + "/cards/" + card_filter, query_params=filters
        )
    ...
    def get_card(self, card_id):
        ...
        """
        json_obj = self.client.fetch_json("/boards/" + self.id + "/cards/" + card_id)
    ...
    def normal_members(self):
        ...
        :rtype: list of Member
        """
        filters = {"filter": "normal", "fields": "all"}
        return self.get_members(filters)
    ...
    def admin_members(self):
        ...
        :rtype: list of Member
        """
        filters = {"filter": "admins", "fields": "all"}
        return self.get_members(filters)
    ...
    def owner_members(self):
        ...
        :rtype: list of Member
        """
        filters = {"filter": "owners", "fields": "all"}
        return self.get_members(filters)
    ...
    def get_members(self, filters=None):
        ...
        """
        json_obj = self.client.fetch_json(
            "/boards/" + self.id + "/members", query_params=filters
        )
        members = list()
        for obj in json_obj:
    ...
    def add_member(self, member, member_type="normal"):
        json_obj = self.client.fetch_json(
            "/boards/{}/members/{}".format(self.id, member.id),
            http_method="PUT",
    ...
    def fetch_actions(self, action_filter, action_limit=50, before=None, since=None):
        ...
        json_obj = self.client.fetch_json(
            "/boards/" + self.id + "/actions", query_params=query_params
        )
    ...
    def get_last_activity(self):
        ...
        """
        json_obj = self.client.fetch_json("/boards/{}/dateLastActivity".format(self.id))
        if json_obj["_value"]:
    ...
    def get_power_ups(self, board_id=None, name="", filters=None):
        ...
        if board_id is None:
            board_id = self.id
        if filters:
            if filters not in ("enabled", "available"):
                pass
    ...
    def enable_power_up(self, powerup_id, board_id=None):
        ...
        warn(
            "Atlassian has DEPRECATED enabling power ups! enable_power_up will be updated once an alternative becomes available",
            DeprecationWarning,
            stacklevel=2,
        )
        if board_id is None:
    ...
    def disable_power_up(self, powerup_id, board_id=None):
        ...
        warn(
            "Atlassian has DEPRECATED disabling power ups! disable_power_up will be updated once an alternative becomes available",
            DeprecationWarning,
            stacklevel=2,
        )
        if board_id is None:
            board_id = self.id
        json_obj = self.client.fetch_json(
            "boards/" + board_id + "/boardPlugins" + powerup_id, http_method="DELETE"
        )
        return json_obj

    def get_enabled_power_ups(
        self,
        board_id=None,
        name="",
    ):
        ...
        if board_id is None:
            board_id = self.id
        json_obj = self.client.fetch_json(
            "boards/" + board_id + "/boardPlugins", http_method="GET"
        )
        print(json_obj)
        return list([PowerUp.from_json(self, json_obj=json) for json in json_obj])
