...
class TrelloClient:
    ...
    def info_for_all_boards(self, actions):
        ...
        if self.public_only:
            ...
            json_obj = self.fetch_json(
                "/members/me/boards/all", query_params={"actions": actions}
            )
    ...
    def get_card(self, card_id):
        ...
        card_json = self.fetch_json(
            "/cards/" + card_id, query_params={"customFieldItems": "true"}
        )
    ...
    def create_hook(self, callback_url, id_model, desc=None, token=None):
        ...
        data = {"callbackURL": callback_url, "idModel": id_model, "description": desc}

        if self.api_key is not None:
            data.update({"key": self.api_key})

        response = self.http_service.post(
            url, data=data, auth=self.oauth, proxies=self.proxies
        )
    ...
    def search(
        ...
        for card_json in json_obj.get("cards", []):
            ...
            if card_json["idBoard"] not in board_cache:
                board_cache[card_json["idBoard"]] = Board(self, card_json["idBoard"])
            ...
            results.append(Card.from_json(board_cache[card_json["idBoard"]], card_json))
    ...
    def add_star(self, board_id, position="bottom"):
        ...
        obj = self.fetch_json(
            "members/me/boardStars", http_method="POST", post_args=post_args
        )
    ...
    def delete_star(self, star):
        ...
        """
        self.fetch_json(
            "members/me/boardStars/{}".format(star.id), http_method="DELETE"
        )
        return star
