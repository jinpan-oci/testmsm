...
class Card(TrelloBase):
    ...
    def __repr__(self):
        return force_str("<Card %s>" % self.name)
    ...
    def fetch_custom_fields(self, json_obj=None):
        ...
        return CustomField.from_json_list(self, json_obj.get("customFieldItems", {}))
    ...
    def fetch_checklists(self):
        ...
        for cl in json_obj:
            checklists.append(Checklist(self.client, cl, trello_card=self.id))
        return checklists

    def fetch_plugin_data(self):
        items = self.client.fetch_json("/cards/" + self.id + "/pluginData")
        return items
    ...
    def fetch_actions(
        self, action_filter="createCard", since=None, before=None, action_limit=50
    ):
        """
    ...
    def _list_movements(self, movement_function, filter_by_date_interval=None):
        ...
        action_since = (
            None if not filter_by_date_interval else filter_by_date_interval[0]
        )
        action_before = (
            None if not filter_by_date_interval else filter_by_date_interval[1]
        )
    ...
    def list_movements(self, list_cmp=None, filter_by_date_interval=None):
        ...
        return self._list_movements(
            movement_function=movement_as_dict_function,
            filter_by_date_interval=filter_by_date_interval,
        )

    # Conversion of units
    def second_to_time_unit(self, time, time_unit):
    ...
    def get_stats_by_list(
        ...
        stats_by_list = {
            list_.id: {"time": 0, "forward_moves": 0, "backward_moves": 0}
            for list_ in lists
        }
        ...
        if len(changes) == 0:
            card_life_time = self.seconds_to_time_unit(
                (datetime.datetime.now(tz) - last_action_datetime).total_seconds(),
                time_unit,
            )
            ...
            for change in ordered_changes:
                ...
                time_from_last_list_change = self.seconds_to_time_unit(
                    (change_datetime - last_action_datetime).total_seconds(), time_unit
                )
            ...
            if (
                ...
                and last_list
                and last_list["id"]
                and last_list["id"] in stats_by_list
                and last_list["id"] != done_list.id
            ):
                time_card_has_spent_in_list_until_now = self.seconds_to_time_unit(
                    (datetime.datetime.now(tz) - last_action_datetime).total_seconds(),
                    time_unit,
                )
                stats_by_list[last_list["id"]][
                    "time"
    ...
    def created_date(self):
        ...
        if not hasattr(self, "creation_date"):
            ...
            self.creation_date = localtz.localize(
                datetime.datetime.fromtimestamp(int(self.id[0:8], 16))
            )
    ...
    def set_custom_field(self, value, custom_field):
        ...
        if custom_field.field_type in ["text", "number", "date", "checkbox"]:
            ...
            if value == "":
                ...
                list_field_id = [
                    x for x, y in custom_field.list_options.items() if y == value
                ][0]
    ...
    def delete(self):
        ...
        self.client.fetch_json("/cards/" + self.id, http_method="DELETE")
    ...
    def unassign(self, member_id):
        self.client.fetch_json(
            "/cards/" + self.id + "/idMembers/" + member_id, http_method="DELETE"
        )
    ...
    def remove_label(self, label):
        self.client.fetch_json(
            "/cards/" + self.id + "/idLabels/" + label.id, http_method="DELETE"
        )
    ...
    def remove_member(self, member):
        self.client.fetch_json(
            "/cards/" + self.id + "/idMembers/" + member.id, http_method="DELETE"
        )
    ...
    def change_board(self, board_id, list_id=None):
        ...
        if list_id is not None:
            args["idList"] = list_id
        self.client.fetch_json(
            "/cards/" + self.id + "/idBoard", http_method="PUT", post_args=args
        )
