...
class CustomFieldDefinition(TrelloBase):
    ...
    def __init__(
        self, client, customFieldDefinition_id, name, field_type, list_options
    ):
        super().__init__()
        self.client = client
    ...
    def __repr__(self):
        return force_str("<CustomFieldDefinition %s>" % (self.name,))
...
class CustomField(TrelloBase):
    ...
    def from_json_list(cls, card, json_objs):
        return [
            cls.get_class(card.board, obj).from_json(card, obj) for obj in json_objs
        ]

    def __repr__(self):
        return force_str(
            "<CustomField%s %s=%r>" % (self.type.capitalize(), self.name, self.value)
        )
...
class CustomFieldCheckbox(CustomField):
    ...
    def from_json(cls, card, json_obj):
        ...
        customField = cls(
            ...
            customField_id=json_obj["id"],
            customFieldDefinition_id=json_obj["idCustomField"],
            value=(json_obj["value"]["checked"] == "true"),
        )
        return customField
...
class CustomFieldDate(CustomField):
    ...
    def value(self, value):
        ...
        assert isinstance(value, str) or isinstance(
            value, unicode
        ), "Given value is no str or unicode!"
...
class CustomFieldNumber(CustomField):
    ...
    def value(self, value):
        ...
        assert isinstance(value, int) or isinstance(
            value, float
        ), "Given value is no int or float!"
        self.client.fetch_json(
            "/card/" + self.card.id + "/customField/" + self.definition_id + "/item",
            http_method="PUT",
            post_args={
