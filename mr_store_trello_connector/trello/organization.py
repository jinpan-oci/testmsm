...
class Organization(TrelloBase):
    ...
    def from_json(cls, trello_client, json_obj):
        ...
        organization = Organization(
            trello_client, json_obj["id"], name=json_obj["name"]
        )
    ...
    def __repr__(self):
        return force_str("<Organization %s>" % self.name)
    ...
    def get_members(self):
        json_obj = self.client.fetch_json(
            ...
            query_params={"filter": "all", "fields": "id,fullName,username,initials"},
        )
        return [
    ...
    def add_member(self, member, member_type="normal"):
        json_obj = self.client.fetch_json(
            "/organizations/{}/members/{}".format(self.id, member.id),
            http_method="PUT",
